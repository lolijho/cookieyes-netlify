...
const db = createClient({
  url: process.env.STORAGE_TURSO_DATABASE_URL || process.env.TURSO_DATABASE_URL,
  authToken: process.env.STORAGE_TURSO_AUTH_TOKEN || process.env.TURSO_AUTH_TOKEN,
});

async function initTursoRemote() {
  console.log('🚀 Inizializzazione database Turso remoto...');
  
  // Verifica connessione
  if (!process.env.STORAGE_TURSO_DATABASE_URL && !process.env.TURSO_DATABASE_URL) {
    console.error('❌ TURSO_DATABASE_URL non configurato');
    process.exit(1);
  }
  
  if (!process.env.STORAGE_TURSO_AUTH_TOKEN && !process.env.TURSO_AUTH_TOKEN) {
    console.error('❌ TURSO_AUTH_TOKEN non configurato');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('👑 Admin: lorecucchini@gmail.com / admin123');
    console.log('👤 Demo: demo@cliente.com / demo123');
